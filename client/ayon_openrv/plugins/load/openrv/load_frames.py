...
class FramesLoader(load.LoaderPlugin):
    ...
    def _register_with_rv_operations(self, node, filepath, context):
        ...
        print(f"\n🚀 [load_frames] _register_with_rv_operations called")
        print(f"   node: {node}")
        ...
        try:
            ...
            print(f"📦 [load_frames] Building event data...")
            event_data = build_event_data_with_versions(context, filepath, self.log)
            event_data['node'] = node
            
            print(f"📤 [load_frames] Sending ayon_source_loaded event...")
            ...
            print(f"✅ [load_frames] Event sent successfully with {version_count} versions")
            self.log.info(f"Fired ayon_source_loaded event with {version_count} versions")
            
        except Exception as e:
            print(f"❌ [load_frames] Error: {e}")
            import traceback
            traceback.print_exc()
